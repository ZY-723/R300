...
int main(int argc, char** argv){
  ...
  ros::Publisher slave_vel = node.advertise<geometry_msgs::Twist>("cmd_vel", 10); //发布从车原始速度
  ros::Subscriber vel_sub = node.subscribe("/ugv2/multfodom", 1, vel_Callback);
...
//中间变量
  ...
  while (node.ok()){
    ...
    else if(multi_mode==2)
    {
        //读取从车位置坐标
      try{  
        listener2.waitForTransform("odom", "base_link", ros::Time(0), ros::Duration(3.0));

        listener2.lookupTransform("odom",  "base_link", ros::Time(0), transformSM2);
      }
      catch (tf::TransformException &ex) {
        ROS_WARN("%s",ex.what());
        ros::Duration(1.0).sleep();
      ...
      printf("Deviation: (e_x = %f e_y = %f e_z =%f)\n",e_linear_x,e_linear_y,e_angular_z);
      printf("主车pose:(%.2f,%.2f,%.2f),主车vel:(%.2f,%.2f,%.2f)\n",trans_x,trans_y,trans_z,odom_linear_x,odom_linear_y,odom_angular_z);
