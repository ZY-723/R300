...
//      bms_status.Warning_Status_2 = state.Warning_Status_2;
                                               ...
                                               double dt)
  {
    ...
    // 1X INS can be the authoritative localisation source. In that case disable wheel-odom TF
    // to avoid two publishers simultaneously broadcasting odom -> base_link.
    if (odom_pub)
    {
